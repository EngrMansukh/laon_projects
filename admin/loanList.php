...
$sql_row ="    SELECT 
    ...
    l.create_date, 
    u.user_name,   
     l.deposite AS IndividualDeposite,   
    SUM(l.withdraw) AS Withdraw,
     u.amount AS Remaining  
FROM loan l 
LEFT JOIN users u ON l.user_id = u.id 
GROUP BY l.user_id, u.user_name, l.create_date, u.amount,IndividualDeposite
ORDER BY l.user_id;";
...
<?php 
  ...
  ?>
      <tr>
        ...
        <td><?=  $sql_row["IndividualDeposite"]; ?></td>
    
        <td><?= $sql_row ['Withdraw']; ?></td>
        <td><?= $sql_row ['Remaining']; ?></td>
